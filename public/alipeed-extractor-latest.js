/* ============================================================
   알리피드 주문 추출 + Airtable 자동 푸시 v2.9.13 (콘솔 fetch 전 페이지 순회)
   ★버전 위생(3곳 항상 일치): ① 파일명(alipeed-extractor-v2.9.13.js) ② 이 헤더 버전 ③ EXTRACTOR_VERSION 상수.
...
(async function(){
  ...
  var EXTRACTOR_VERSION = 'v2.9.13';   // ★파일명·헤더와 항상 일치시킬 것
  ...
  Promise.all(detailPromises).then(function() {
    ...
    saseomList.forEach(function(s) {
      ...
      statsBySaseom[s] = {
        ...
        통관고유부호: (arr.filter(function(o){ return o.전자상거래 && o.통관고유부호; })[0] || {}).통관고유부호 || ''   // [v2.9.13] 전자상거래 그룹 첫 부호값(동일 사서함=동일 회원=동일 부호).
      };
    });
      ...
      return atFetchExistingByKey(pat, AT_TABLES.Shipments, '사서함', compositeList)
       .then(function(existingShipMap) {
        var shipmentRecords = saseomList.map(function(s) {
          ...
          // [v2.9.13] 통관고유부호 → Shipments.수취_통관부호(매니페스트 L 소스). 전자상거래+값 있을 때만(무역·빈값 미포함).
          if (st.전자상거래 && st.통관고유부호) fields['수취_통관부호'] = st.통관고유부호;
