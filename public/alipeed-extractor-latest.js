/* ============================================================
   알리피드 주문 추출 + Airtable 자동 푸시 v2.9.14 (콘솔 fetch 전 페이지 순회)
   ★버전 위생(3곳 항상 일치): ① 파일명(alipeed-extractor-v2.9.14.js) ② 이 헤더 버전 ③ EXTRACTOR_VERSION 상수.

   v2.9.14 수정 (2026-07-24): ★통관고유부호·사업자개인 저장 위치 = Orders(주문·수취인 단위).
     - v2.9.13은 Shipments.수취_통관부호(사서함 대표값) 저장 → 다른 수취인 부호 오적용(통관사고) → 제거.
     - Orders.수취_통관부호(부호)·Orders.사업자개인(사업자/개인) 저장. 전자상거래+값 있을 때만·빈칸만·single select는 사업자/개인만.
     - Shipments.사업자개인은 유지(도구 계산·부킹요약·알리코인 판정 사서함 대표값). 매니페스트 L·M은 Orders 우선.
   v2.9.13 수정 (2026-07-24): ★통관고유부호 추출 + 사업자개인 부호 폴백.
     - 파란 줄 괄호 안 부호(예 "전자상거래(사업자)-해운 (4288102340)") 파싱 → Shipments.수취_통관부호(매니페스트 L 소스).
     - 사업자개인 판별 폴백: ①사업자/개인 텍스트 ②부호 접두(P=개인, 숫자10=사업자). 임의 대입 없음(둘 다 없으면 빈값).
     - ★기존값 덮어쓰기 안 함(전자상거래+값 있을 때만 저장). 재추출 아닌 신규 07-27 추출분에 자연 반영.
...
(async function(){
  ...
  var EXTRACTOR_VERSION = 'v2.9.14';   // ★파일명·헤더와 항상 일치시킬 것
  ...
  function parseOrderRow($root, seq) {
    ...
    // [v2.9.13] 통관고유부호 파싱 — 알리피드 파란 줄 괄호 안(예 "전자상거래(사업자)-해운 (4288102340)").
    //   (사업자)/(개인) 한글 괄호 제외 → P접두 or 숫자6+ 만. 매니페스트 L열 소스(수취_통관부호에 저장).
    var _custCodeMatch = ordTyText.match(/[(（]\s*(P?\d{6,})\s*[)）]/);
    var customsUniqueNo = _custCodeMatch ? _custCodeMatch[1] : '';
    ...
    // [v2.9.13] 사업자개인 판별 우선순위: ①ordTyText '사업자'/'개인' 텍스트 ②통관고유부호 접두(P=개인, 숫자10=사업자). 둘 다 없으면 빈값(임의 대입 금지).
    var bizPersonal = /사업자/.test(ordTyText) ? '사업자'
                    : (/개인/.test(ordTyText) ? '개인'
                    : (/^P/.test(customsUniqueNo) ? '개인'
                    : (/^\d{10}$/.test(customsUniqueNo) ? '사업자' : '')));
    var customsBucket = '';
    if (isEcommerce) {
    ...
    return {
      ...
      통관고유부호: customsUniqueNo,   // [v2.9.13] 괄호 안 부호 → Shipments.수취_통관부호(매니페스트 L 소스).
  ...
  Promise.all(detailPromises).then(function() {
    ...
    saseomList.forEach(function(s) {
      ...
      statsBySaseom[s] = {
        ...
        사업자개인: (arr.filter(function(o){ return o.전자상거래 && o.사업자개인; })[0] || {}).사업자개인 || ''   // [v2.9.10] 전자상거래 그룹은 통관구분(사업자/개인 포함)으로 분리 → 동일값. 첫 전자상거래 주문값 채택. (통관고유부호는 Orders 단위 저장 — v2.9.14)
      };
    });
      ...
      return atFetchExistingByKey(pat, AT_TABLES.Shipments, '사서함', compositeList)
       .then(function(existingShipMap) {
        var shipmentRecords = saseomList.map(function(s) {
          ...
          if (st.전자상거래 && st.사업자개인) fields['사업자개인'] = st.사업자개인;   // 사서함 대표값(도구 계산·부킹요약·알리코인 판정용). 매니페스트 L·M은 Orders 우선(v2.9.14).
          // [v2.9.14] ★Shipments.수취_통관부호 저장 제거 — 사서함 대표값은 다른 수취인 부호 오적용(통관사고). 통관고유부호는 Orders에만 저장.
          ...
          return atFetchExistingByKey(pat, AT_TABLES.Orders, '주문번호', orderNos, ['주문상태', 'Shipment'])
           .then(function(existingOrderMap) {
            ...
            var orderRecords = orders.map(function(o) {
              ...
              // [v2.9.14] ★통관고유부호·사업자개인 = 주문(수취인) 단위 → Orders 저장(매니페스트 1행=1주문 일치).
              //   전자상거래 건만·값 있을 때만(빈값·무역 미포함=기존 보존). ★사업자개인은 '사업자'/'개인'만(single select 오염 방지).
              if (o.전자상거래 && o.통관고유부호) fields['수취_통관부호'] = o.통관고유부호;
              if (o.전자상거래 && (o.사업자개인 === '사업자' || o.사업자개인 === '개인')) fields['사업자개인'] = o.사업자개인;
